    ...
    .then((result) => {
      ...
      result.forEach((item, index) => {
        ...
        if (index % 2 === 0) {
          ...
          newItem.querySelector(".mescont").textContent = "You recieved a message";
        } else if (index % 2 === 1) {
...
function getPackageID(orderID) {
  ...
  return fetch(BASE_URL + `/order?orderId=${orderID}`, requestOptions)
    ...
    .then((data) => {
      ...
      const designerId = data[0].designerId; // Access the packageId property from the first object in the array
...
getPackageID(orderId)
  .then((result) => {
    ...
    const { packageID, Price, designerId, createdTime, requirements, status } =
    ...
    return getDeliveryDuration(packageID).then((deliveryDuration) => {
      ...
      return getClientName(designerId).then((designerName) => {
        return {
          deliveryDuration,
          Price,
          designerName,
...
function getClientName(UserID) {
  ...
  return fetch(BASE_URL + `/user?userId=${UserID}`, requestOptions)
    .then((response) => {
      if (!response.ok) {
        throw new Error("Network response was not ok");
      }
